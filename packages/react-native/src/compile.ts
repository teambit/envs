...
import { FIXED_OUT_DIR, getBabelrc } from './babelrc';
...
async function preCompile(context: CompilationContext, preset: Preset) {
  return preset.preCompile
    ? Promise.all([createBabelConfig(context), preset.preCompile(context)])
    : createBabelConfig(context);
}
...
export function findMainFile(context: CompilationContext, dists: Vinyl[]) {
  ...
  const sourceFileName = getNameOfFile(context.main, '.js');
...
async function collectNonDistFiles(context: CompilationContext): Promise<Vinyl[]> {
  ...
  const ignoreFunction = function(file: string, _stats: Stats) {
    const defaultIgnore = [`node_modules${path.sep}`, FIXED_OUT_DIR, '.dependencies', '.jsx'];
    return defaultIgnore.concat(copyPolicy.ignorePatterns).reduce(function(prev, curr) {
      return prev || !!~file.indexOf(curr);
    }, false);
  };
  const fileList = await readdir(capsuleDir, ['*.js', ignoreFunction]);
  const readFiles = await Promise.all(
    fileList.map(file => {
      return fs.readFile(file);
    })
...
function getBabelConfigPath(context: CompilationContext) {
  return path.join(context.directory, '.babelrc');
}
